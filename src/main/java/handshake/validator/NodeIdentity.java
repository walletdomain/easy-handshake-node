...
/**
 * NodeIdentity — manages the validator's persistent Brontide keypair.
    * <p>
 * The validator key is a 32-byte secp256k1 private key stored in `validator.key`
 * in the data directory. It is generated once and reused across restarts.
    * <p>
 * The corresponding compressed public key is the validator's identity on the
...
public class NodeIdentity {
    ...
    private NodeIdentity(String dataDir) {
        Path keyPath = Path.of(dataDir, "validator.key");
        byte[] privKey;
        ...
        if (Files.exists(keyPath)) {
            try {
                ...
                privKey = fromHex(hex);
                System.out.println("[Identity] Loaded validator key from " + keyPath);
            } catch (IOException e) {
                throw new RuntimeException("Failed to read validator.key: " + e.getMessage(), e);
            }
        } else {
            // Generate new key
            privKey = new byte[32];
            ...
            try {
                ...
                System.out.println("[Identity] Generated new validator key at " + keyPath);
            } catch (IOException e) {
                throw new RuntimeException("Failed to write validator.key: " + e.getMessage(), e);
            }
        }
